...
import { Cloud, Trash2, DownloadCloud, Search, Navigation } from 'lucide-react';
import './index.css';
...
function App() {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const res = await axios.post('http://localhost:5000/weather/query', {
        location,
        startDate,
        endDate
      });
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/weather/query/${id}`);
  ...
  const exportCSV = () => {
    window.open('http://localhost:5000/weather/export/csv', '_blank');
    showToast("CSV Downloaded!", "success");
  };

  const exportJSON = () => {
    window.open('http://localhost:5000/weather/export/json', '_blank');
    ...
    <div className="min-h-screen bg-slate-950 bg-[radial-gradient(ellipse_80%_80%_at_50%_-20%,rgba(120,119,198,0.3),rgba(255,255,255,0))] text-white p-6 pb-20 relative">
      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        <header className="text-center pt-10">
          <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-blue-400 to-cyan-300 bg-clip-text text-transparent flex items-center justify-center gap-4">
